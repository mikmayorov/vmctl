...
def has_netbox_key(config: dict) -> bool:
    ...
    return bool(os.environ.get("NETBOX_TOKEN") or (netbox.get("key") if isinstance(netbox, dict) else None))
...
def _settings(config: dict) -> tuple[str, str]:
    ...
    if not isinstance(token, str) or not token:
        raise NetBoxError("set netbox.key in config.toml or NETBOX_TOKEN")
...
def find_device(config: dict) -> tuple[int, int]:
    device = config.get("netbox", {}).get("device")
    if type(device) is int and device > 0:
        ...
        listing = _request(config, "GET", f"dcim/devices/?{query}")
        matches = [item for item in listing.get("results", []) if item.get("name") == device]
        if len(matches) != 1:
            raise NetBoxError(f"expected one NetBox device named {device!r}, found {len(matches)}")
        result = matches[0]
    else:
...
def find_vm(config: dict, name: str, cluster_id: int) -> dict | None:
    ...
    result = _request(config, "GET", f"virtualization/virtual-machines/?{query}")
    matches = [
...
def list_vms(config: dict, device_id: int) -> list[dict]:
    ...
    while path:
        ...
        records.extend(vm for vm in page.get("results", []) if
